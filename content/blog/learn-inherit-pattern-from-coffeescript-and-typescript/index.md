...
それはさておき、ここで重要なのが、継承のための関数`inherit()`です。
...
js にコンパイルする際に、**特定の表現と、継承するための汎用関数を吐き出し**て、それを利用しています。
...
肝心の継承する関数が、`__extends(child, parent)`です。
...
そしてさりげなく、\_\_extends という**関数が未定義の時のみ独自定義する**といった気配りが入っています。

TypeScript で継承する関数`__extends()`では、  
...
継承する子クラスでは、即時関数の引数に`_super`を取っています。  
この\_super は、親クラスのコンストラクタ関数を指しています。  
この親コンストラクタ関数を、継承する関数**extends を通じて、 子クラスの**super\_\_オブジェクトに設定しています。
