...
更に、コールバック関数では、配列ではなく渡したPromiseが１つずつ引数として渡ってきます。  
いちいちバベって`.then(([a, b, c]))`のように引数を展開する必要が無いので、予め決まっている複数個のPromiseを待つ処理に便利です
...
更に、**インスタンスメソッドとしても実行可能**なので、Promiseベースのパイプラインを作りたいときなどにとても流暢に書けるようになります
