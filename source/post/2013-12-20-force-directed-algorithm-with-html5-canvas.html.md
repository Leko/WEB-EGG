...
こいつを JavaScript(CoffeeScript)と canvas で実装してみました。
...
力学モデルの Wiki に書いてある通りですが、少し噛み砕いてみます。
...
Wiki に書いてある擬似コード、ほぼそのままだと思います。

## Canvas ユーティリティの作成

今回は canvas で実装するので、さくっと canvas のユーティリティを作ります。
...
最初の方はあまり意味がなく、canvas のサイズ調整をしているだけです。

いくつかノードを生成して、  
そのノードのインデックスを渡して接続します。

そして、そのグラフのインスタンスを canvas に add（↑ の Canvas クラスを参照）して、  
50 ミリ秒ごとに再描画をかけています。
