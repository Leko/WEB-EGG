...
今日は PHP のマジックメソッドについて書きます。

PHP のマジックメソッドの中に`__callStatic`というメソッドがあります。  
これは、クラスで定義されていないメソッドに対して static なコールをした際に呼び出されるフックです。

この機能を使えば、Ruby でいうところの`method missing`のような挙動が可能になるのでは？  
と考え、実験に Ruby on Rails で以前まで使われていた`find_all_by_*`を実装してみたいと思います。
...
## \_\_callStatic の仕様を読む

> \_\_callStatic() は、 アクセス不能メソッドを静的コンテキストで実行したときに起動します。
>
...
サンプルコードを呼んでみると、`__callStatic`は PHP 5.3 以上で動作可能な機能のようです。
